...
object Stringify {
  ...
  private def stringifyHelp(data: Any, path: Stack[String], idMap: IdentityHashMap[Any, Int], replacer: StringifyReplacer): String =
        ...
        data match {
          ...
          case _ => {
            if(idMap.containsKey(data)) {
              ...
              throw new Exception(s"circle detected! value('${pathSeq.mkString(".")}') is the same as value('${pathSeq.slice(0, idx).mkString(".")}').")
            }
            idMap.put(data, path.length)
            val ret = data match {
              // array
              case v: AbstractSeq[Any] => stringifyList(v, path, idMap, replacer) 
              case v: scala.collection.mutable.AbstractSeq[Any] => stringifyList(v.toList, path, idMap, replacer) 
              case v: Array[Any] => stringifyList(v.toList, path, idMap, replacer)

              // object
              case v: scala.collection.mutable.Map[String, Any] => stringifyTupleList(v.toList, path, idMap, replacer)
              case v: scala.collection.immutable.Map[String, Any] => stringifyTupleList(v.toList, path, idMap, replacer)
  ...
  private def stringifyList(v: AbstractSeq[Any], path: Stack[String], idMap: IdentityHashMap[Any, Int], replacer: StringifyReplacer) = {
    s"[${v.zipWithIndex.map((tup) => {
      path.push(tup._2 + "")
  ...
  private def stringifyTupleList(tupleList: List[(Any, Any)], path: Stack[String], idMap: IdentityHashMap[Any, Int], replacer: StringifyReplacer) = {
    s"{${tupleList.map((item) => {
